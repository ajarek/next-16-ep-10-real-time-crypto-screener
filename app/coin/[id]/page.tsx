  ...
  return (
    <div className='min-h-[calc(100vh-64px)] w-full grid grid-cols-[1fr] md:grid-cols-[1fr_1fr] lg:grid-cols-[1fr_1fr_1fr] gap-2 border-4 rounded-lg px-16 py-8 place-items-center'>
      ...
      <div className='flex flex-col items-start gap-2'>
        ...
        <p>
          Market Cap Rank:{" "}
          <span className='font-semibold text-xl'>{coin?.market_cap_rank}</span>
        </p>
        <p>
      ...
      <div className='flex flex-col items-start gap-2'>
        ...
        <Button disabled={!isAuthenticated} className={`${!isAuthenticated ? "cursor-not-allowed" : "cursor-pointer"}`}>Add to watchlist</Button>
      </div>
      </div>
    </div>
  )
